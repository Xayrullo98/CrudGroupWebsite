...
def add_trade(form: TradeCreate, db: Session = Depends(get_db),current_user: UserCurrent = Depends(get_current_active_user)):  # current_user: CustomerBase = Depends(get_current_active_user)
...
def get_order_id(order_id: int, db: Session = Depends(get_db),current_user: UserCurrent = Depends(get_current_active_user)):
	return get_order_id_from_trades(id=order_id, user=current_user, db=db)
